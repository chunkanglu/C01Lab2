  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (result.deletedCount === 0) {
        return res.status(404).send(`Note with ${noteId} belonging to the user ${decoded.username} not found.`);
      }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    const { title, content } = req.body;
    if (!title && !content) {
      return res.status(400).json({ error: "No title nor content" });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (result.modifiedCount === 0) {
        return res
          .status(404)
          .json({ error: `Note with ID ${noteId} belonging to the user ${decoded.username} not found.` });
      }
